...
@app.route("/pokemon/<pokemonname>", methods=["GET"])
def show_pokemon(pokemonname):
    ...
    for item in pokemon:
        if item["name"].lower() == pokemonname.lower():
            result.append(item)

    if not result:
        return f"No Pokemon found with name: {pokemonname}"

    return render_template('pokemonflaskapi.html', pokemonname=pokemonname, pokemon=result)
